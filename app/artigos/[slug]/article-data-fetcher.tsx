// app/artigos/[slug]/article-data-fetcher.tsx

// Este é um Server Component, não precisa de 'use client'
// Ele é responsável por buscar os dados do artigo.
...
// --- Configuração do Image URL Builder ---
...
// --- Função Assíncrona para Buscar um Post Específico do Sanity (Server-side) ---
async function getPost(slug: string) {
  ...
  // Força a busca dinâmica no servidor para garantir dados frescos
  const post = await client.fetch(query, {}, { cache: 'no-store' }); 
  return post;
}

// --- Componente Server-side que busca os dados e passa para o Client Component ---
interface ArticleFetcherProps {
  slug: string;
}

export default async function ArticleFetcher({ slug }: ArticleFetcherProps) {
  const post = await getPost(slug); // Busca o post no servidor
